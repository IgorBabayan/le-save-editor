...
internal partial class DownloadViewModel : ObservableObject, IDownloadViewModel
{
	...
	{
		Dispatcher.UIThread.Post(() => DownloadProgressText = Path.GetFileName(path));
		...
		{
			Dispatcher.UIThread.Post(() => DownloadProgress++);
		}
	}

	private async Task ConvertFile(string path)
	{
		Dispatcher.UIThread.Post(() => ConvertProgressText = Path.GetFileName(path));
		...
		{
			Dispatcher.UIThread.Post(() => ConvertProgress++);
		}
	}

	private async Task DeleteFile(string path)
	{
		Dispatcher.UIThread.Post(() => RemoveProgressText = Path.GetFileName(path));
		...
		{
			Dispatcher.UIThread.Post(() => RemoveProgress++);
		}
	}
}
